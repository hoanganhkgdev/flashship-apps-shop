...
void main() async {
  ...
  await FirebaseMessaging.instance.setForegroundNotificationPresentationOptions(
    // Chỉ hiện notification hệ thống khi app ở background. Khi foreground,
    // NotificationService vẫn nhận FCM để cập nhật dữ liệu/trạng thái đơn
    // nhưng không làm gián đoạn người dùng bằng banner hoặc âm thanh.
    alert: false,
    badge: false,
    sound: false,
  );
...
class _FlashShipShopAppState extends ConsumerState<FlashShipShopApp>
  ...
  Future<void> _onSessionExpired() async {
    ...
    try {
      ...
      if (ctx != null && ctx.mounted) {
        AppSnackbar.error(
            ctx, 'Phiên đăng nhập đã hết hạn, vui lòng đăng nhập lại');
      }
  ...
  Future<void> _maybeShowPinLock() async {
    ...
    await Navigator.of(ctx, rootNavigator: true).push(
      MaterialPageRoute(
          fullscreenDialog: true, builder: (_) => const PinLockScreen()),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<AppVersionState>(appVersionProvider, (prev, next) {
      if (next.needsForceUpdate) {
        WidgetsBinding.instance
            .addPostFrameCallback((_) => _showForceDialog(next));
      }
    });
    ...
    return MaterialApp.router(
      ...
      builder: (context, child) {
        ...
        final scale = media.textScaler.scale(1) * 1.08;
        return MediaQuery(
